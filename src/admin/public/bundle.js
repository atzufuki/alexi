...
// src/myapp/src/backends/rest_backend.ts
var TOKEN_STORAGE_KEY = "myapp_sync_tokens";
...
// src/myapp/src/backends/sync_backend.ts
...
// src/myapp/src/backends/setup.ts
...
function getEnvConfig() {
  ...
  if (typeof Deno !== "undefined" && Deno.env) {
    try {
      const backend = Deno.env.get("MYAPP_BACKEND");
      if (backend === "sync" || backend === "indexeddb") {
        config11.backend = backend;
      }
      const apiUrl = Deno.env.get("MYAPP_API_URL");
      if (apiUrl) {
        config11.apiUrl = apiUrl;
      }
      const databaseName = Deno.env.get("MYAPP_DATABASE_NAME");
      if (databaseName) {
        config11.databaseName = databaseName;
      }
      const debug = Deno.env.get("MYAPP_DEBUG");
      if (debug !== void 0) {
        config11.debug = debug === "true";
      }
      const failSilently = Deno.env.get("MYAPP_FAIL_SILENTLY");
  ...
  if (typeof window !== "undefined") {
    const windowConfig = window.__MYAPP_CONFIG__;
    if (windowConfig) {
