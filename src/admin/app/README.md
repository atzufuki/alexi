...
`alexi_admin`. It follows the same patterns as the main `myapp` frontend application.
...
The admin SPA is served by the `adminMiddleware` in `myapp-web`. When a request comes to
`/admin/*`:
